...
def test_msi_driver_writes_expected_packet() -> None:
...
def test_msi_driver_skips_unchanged_zone_colors() -> None:
    ...
    driver = MsiMysticHidDriver(
        config=MsiMysticHidConfig(hid_path="test-path", pad_length=8, zone_count=24),
        writer=fake_writer,
    )
...
def test_msi_driver_rejects_zone_outside_configured_count() -> None:
    driver = MsiMysticHidDriver(
        config=MsiMysticHidConfig(hid_path="test-path", pad_length=8, zone_count=4),
        writer=lambda **_: 8,
    )
