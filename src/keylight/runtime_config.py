...
def _validate_live_defaults(defaults: LiveCommandDefaults) -> None:
    ...
    if defaults.mapper == "calibrated" and defaults.zone_profile is None:
    ...
    if defaults.smoothing_alpha < 0.0 or defaults.smoothing_alpha > 1.0:
        raise ValueError("smoothing.alpha must be in range 0.0..1.0.")
